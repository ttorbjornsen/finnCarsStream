...
object KafkaProducer extends App{
  ...
  //val fSer: Future[Unit] = f1.flatMap(res1 => f2)
  ...
  fSer2.onComplete{
    case Success(_) => println("Finished writing pages " + Math.min(hdrPages1.min, hdrPages2.min) + " - " + Math.max(hdrPages1.max, hdrPages2.max) + " to Kafka")
